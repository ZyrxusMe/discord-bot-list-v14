...
module.exports = {
    run: async (client,interaction) =>{
        ...
        sunucudb["rol-bot"] = rol.id

        db.save_data();
        interaction.reply({embeds:[client.embeds.successEmbed(client, "Bot rolü "+ `(<@&${rol.id}>)` +" başarılı bir şekilde ayarlandı.")], ephemeral:true})
    },
    name: "rol-bot",
    description: "Botların hangi role sahip olması gerektiğini ayarlar.",
    type: 1,
    options: [
        {
            name: "rol",
            type: 8,
            description: "Bot rolü.",
            required: true
        }
    ]
}